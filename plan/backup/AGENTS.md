...
Recruiting website for Jacob Deja (#10, center mid). The plan docs in this repo are the source of truth —
read them before writing code, and re-read the relevant section before each phase.

## Read order
1. `README.md` — master TODO (tasks A0–A9, B1–B6), current status, changelog. Start every session here.
2. `DESIGN-SYSTEM-v0.2.md` — visual system: tokens, type, page structure, scrub spec, AE direction.
3. `ARCHITECTURE-DEPLOYMENT-v0.1.md` — stack, file structure, content pipeline, deploy runbook.
4. `AFTER-EFFECTS-GUIDE.md` — footage pipeline (human-executed; code consumes its outputs).
5. `EMAIL-SERVICE-PLAN-v0.1.md` — Track B, separate build, do not start until Track A ships.
...
- Update `README.md` checkboxes and add a line to its changelog when a task completes.

## Working style
- One task from README's Track A at a time, in order (A1 → A9). A4/A5 (the scrub) is the critical path —
...
- Don't restructure the plan docs; append to README's changelog instead.
